...
use crate::flags::error::{FlagsError, ParseError};
...
pub struct Arg {
    ...
    /// Argument description to display in the help command
...
impl fmt::Display for Args {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        for arg in self.0 {
            let width = 10 - arg.long.len();

            if let Some(short) = arg.short {
                writeln!(
                    f,
                    "{:4}-{}, --{}{:width$}{}",
                    "", short as char, arg.long, "", arg.description
                )?;
            } else {
                writeln!(f, "{:8}--{}{:width$}{}", "", arg.long, "", arg.description)?;
            }

            match arg.takes_value {
                TakesValue::Necessary(Some(val)) => {
                    writeln!(f, "{:10}takes a value in [{}]", "", val.join(","))?;
                }
                TakesValue::Optional(Some(val)) => {
                    writeln!(f, "{:10}takes an optional value in [{}]", "", val.join(","))?;
                }
                _ => (),
            }
        }
...
impl<'a> MatchedFlags<'a> {
    pub fn get(&self, arg: &'static Arg) -> Result<Option<&'a OsStr>, FlagsError> {
        self.get_where(|flag| flag.matches(arg))
    }

    pub fn get_where<P>(&self, predicate: P) -> Result<Option<&'a OsStr>, FlagsError>
    where
        P: Fn(&Flag) -> bool,
    {
        if self.is_strict() {
...
fn os_str_to_bytes(os_str: &OsStr) -> &[u8] {
    os_str.as_bytes()
}
...
mod split_on_equal_test {
    ...
    macro_rules! test_split_on_equal {
        ($fn_name:ident: $input:expr => None ) => {
            #[test]
            fn $fn_name() {
                assert_eq!(split_on_equal(&OsString::from($input)), None);
            }
        };
